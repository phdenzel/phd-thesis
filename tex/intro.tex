...
that aim, each designated by a project name: \Code{FOSSIL}~(\chref{fossil}),
\Code{ADLER}~(\chref{adler}), \Code{MATCH}~(\chref{match}), and
\Code{DELAYS}~(\chref{delays}).
...
certain issues and limitations.  The subsequent project in \chref{adler} reports
on a blind test which further explores these problems with mock lenses from
large-scale hydrodynamical simulations.  Particularly hard to solve is the issue
of degeneracies, the fact that a lens configuration can be caused by many,
...
to isolate these problems.  Finally, \chref{match} gives proof-of-concept for an
...
few key aspects in cosmology, some theoretical and observational background
related to galactic dynamics, in particular focusing on topics relevant to
galaxies in lensing systems, and some basic lensing theory.  Finally, the
subsequent chapters are put into context and shortly summarized.
